...
Q= [0.5; 0]; %initized state--it has two components: [position; velocity] of the Quail
...
% NinjaVision_noise_mag = 1;  %measurement noise: How mask-blinded is the Ninja (stdv of location, in meters)
QuailAccel_noise_mag = 0;  % process noise
NinjaVision_noise_mag = 0.0037;  % measurement noise
...
plot(0:dt:duration, smooth(Q_loc_meas), '-g.')
...
Q= [Q_loc_meas(1); 0]; % re-initized state
P_estimate = P;
P_mag_estimate = [];
predic_state = [];
predic_var = [];
...
axis([0 10 min([Q_loc(:); Q_loc_meas(:); Q_loc_estimate(:)]) max([Q_loc(:); Q_loc_meas(:); Q_loc_estimate(:)])])
...
clf
    x = Q_loc_estimate(T)-5:.001:Q_loc_estimate(T)+5; % range on x axis
    ...
    sigma = predic_var(T)+10^-3; % standard deviation
    y = normpdf(x,mu,sigma); % pdf
    y = y/(max(y));
    hl = plot(x,y,'Color','m','LineWidth',2); % or use hold on and normal plot
    
%     disp([mu sigma Q_loc(T)])
...
%     disp([mu sigma Q_loc(T)])
    ...
    subplot(1,2,2)
    plot(tt(1:T),Q_loc(1:T),'-r.',tt(1:T),Q_loc_meas(1:T),'-k.', tt(1:T),Q_loc_estimate(1:T),'-g.');
    pause
end