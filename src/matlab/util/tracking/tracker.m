...
% 	window_sz = 2 .^ nextpow2(window_sz);
  ...
  reader = VideoReader([video_path img_files{1}]);
	time = 0;  %to calculate FPS
	positions = zeros(reader.NumberOfFrames, 2);  %to calculate precision

  hwait = waitbar(0, 'Processing video');
  warning('Processing half the frames')
	for frame = 1:reader.NumberOfFrames/2
    waitbar(frame/reader.NumberOfFrames,hwait)
		%load image
% 		im = imread([video_path img_files{frame}]);
    im = read(reader, frame);
		if size(im,3) > 1,
			im = rgb2gray(im);
		end
		if resize_image,
