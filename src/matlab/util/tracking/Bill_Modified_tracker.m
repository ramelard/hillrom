function [positions, time, imgs] = Bill_Modified_tracker(imgs, pos, target_sz, ...
...
% 	window_sz = 2 .^ nextpow2(window_sz);
	...
	positions = zeros(size(imgs,3), 2);  %to calculate precision
    
%     im = imread([video_path img_files{1}]);
%     imgs = uint8(zeros(size(im,1), size(im,2), numel(img_files)));
%     h = waitbar(0,'Reading images from Disk');
%     for frame = 1:numel(img_files),
%         imgs(:,:,frame) = imread([video_path img_files{frame}]);
%         waitbar(frame / numel(img_files), h);
%     end
%     close(h);
	...
	for frame = 1:size(imgs,3),
        if mod(frame, 25) == 0
            waitbar(frame / size(imgs,3), h);
        end
		%load image
		im = squeeze(imgs(:,:,frame));
