...
if (max(dh-dl)/res>1000), 
  warning('Excessive data range relative to resolution.');
end;
...
% p2 = zeros(size(p,1)+2*size(win,1), size(p,2)+2*size(win,2));
% p2(size(win,1)+1:end-size(win,1),size(win,2)+1:end-size(win,2)) = p;
% p2(size(win,1)+1:end-size(win,1),1:size(win,2)) = repmat(p(:,1), [1 size(win,2)]);
% p2(size(win,1)+1:end-size(win,1),end-size(win,2)+1:end) = repmat(p(:,end), [1 size(win,2)]);
% p2(1:size(win,1),size(win,2)+1:end-size(win,2)) = repmat(p(1,:), [size(win,1) 1]);
% p2(end-size(win,1)+1:end,size(win,2)+1:end-size(win,2)) = repmat(p(end,:), [size(win,1) 1]);
