  ...
  emxArray_real_T *weights, emxArray_int32_T *indices)
{
  ...
  if (rtIsNaN(out_length)) {
    ...
    for (i3 = 0; i3 <= loop_ub; i3++) {
      y->data[y->size[0] * i3] = 1.0 + (double)i3;
    }
  }
  ...
  i3 = u->size[0];
  u->size[0] = y->size[1];
  emxEnsureCapacity((emxArray__common *)u, i3, (int)sizeof(double));
  loop_ub = y->size[1];
  for (i3 = 0; i3 < loop_ub; i3++) {
    u->data[i3] = y->data[y->size[0] * i3] / scale + s;
  }
  ...
  i3 = av->size[0];
  av->size[0] = u->size[0];
  emxEnsureCapacity((emxArray__common *)av, i3, (int)sizeof(double));
  loop_ub = u->size[0];
  for (i3 = 0; i3 < loop_ub; i3++) {
    av->data[i3] = u->data[i3] - s;
  }

  emxInit_real_T2(&bv, 1);
  i3 = bv->size[0];
  bv->size[0] = av->size[0];
  emxEnsureCapacity((emxArray__common *)bv, i3, (int)sizeof(double));
  loop_ub = av->size[0];
  for (i3 = 0; i3 < loop_ub; i3++) {
    bv->data[i3] = av->data[i3];
  }
  ...
  i3 = aux->size[0] * aux->size[1];
  aux->size[0] = 1;
  aux->size[1] = n;
  emxEnsureCapacity((emxArray__common *)aux, i3, (int)sizeof(int));
  if (n > 0) {
    aux->data[0] = 0;
    P = 0;
  ...
  emxEnsureCapacity((emxArray__common *)b_bv, i3, (int)sizeof(int));
  loop_ub = bv->size[0];
  for (i3 = 0; i3 < loop_ub; i3++) {
    b_bv->data[i3] = (int)bv->data[i3];
  }

  emxInit_real_T1(&b_indices, 2);
  bsxfun(b_bv, aux, indices);
  i3 = b_indices->size[0] * b_indices->size[1];
  b_indices->size[0] = indices->size[0];
  b_indices->size[1] = indices->size[1];
  emxEnsureCapacity((emxArray__common *)b_indices, i3, (int)sizeof(double));
  loop_ub = indices->size[0] * indices->size[1];
  emxFree_int32_T(&b_bv);
  for (i3 = 0; i3 < loop_ub; i3++) {
    b_indices->data[i3] = indices->data[i3];
  }

  b_bsxfun(u, b_indices, weights);
  emxFree_real_T(&b_indices);
  if (scale < 1.0) {
    i3 = weights->size[0] * weights->size[1];
    emxEnsureCapacity((emxArray__common *)weights, i3, (int)sizeof(double));
    n = weights->size[0];
    P = weights->size[1];
    loop_ub = n * P;
    for (i3 = 0; i3 < loop_ub; i3++) {
      weights->data[i3] *= scale;
    }
  }
  ...
  loop_ub = n * P;
  for (i3 = 0; i3 < loop_ub; i3++) {
    weights->data[i3] = ((-0.5 <= weights->data[i3]) && (weights->data[i3] < 0.5));
  }
  ...
  if (!((weights->size[0] == 0) || (weights->size[1] == 0))) {
    ...
    n = 0;
    nc1 = weights->size[0];
    i3 = weights->size[0] * weights->size[1] - weights->size[0];
    ck = 0;
    ...
    while (ck <= i3) {
      ...
      i4 = cv->size[0];
      cv->size[0] = av->size[0];
      emxEnsureCapacity((emxArray__common *)cv, i4, (int)sizeof(double));
      loop_ub = av->size[0];
      for (i4 = 0; i4 < loop_ub; i4++) {
        cv->data[i4] = av->data[i4] / bv->data[i4];
      }
  ...
  i3 = indices->size[0] * indices->size[1];
  for (ck = 0; ck < i3; ck++) {
    if (P == 0) {
  ...
  for (i3 = 0; i3 < 2; i3++) {
    outsize[i3] = (unsigned int)weights->size[i3];
  }

  emxInit_boolean_T(&copyCols, 2);
  i3 = copyCols->size[0] * copyCols->size[1];
  copyCols->size[0] = 1;
  copyCols->size[1] = (int)outsize[1];
  emxEnsureCapacity((emxArray__common *)copyCols, i3, (int)sizeof(boolean_T));
  loop_ub = (int)outsize[1];
  for (i3 = 0; i3 < loop_ub; i3++) {
    copyCols->data[i3] = false;
  }
  ...
  n = weights->size[0];
  i3 = b_weights->size[0] * b_weights->size[1];
  b_weights->size[0] = aux->size[1];
  b_weights->size[1] = n;
  emxEnsureCapacity((emxArray__common *)b_weights, i3, (int)sizeof(double));
  for (i3 = 0; i3 < n; i3++) {
    loop_ub = aux->size[1];
    for (i4 = 0; i4 < loop_ub; i4++) {
      b_weights->data[i4 + b_weights->size[0] * i3] = weights->data[i3 +
        weights->size[0] * (aux->data[aux->size[0] * i4] - 1)];
    }
  }
  ...
  for (i3 = 0; i3 < loop_ub; i3++) {
    P = b_weights->size[0];
    for (i4 = 0; i4 < P; i4++) {
      weights->data[i4 + weights->size[0] * i3] = b_weights->data[i4 +
        b_weights->size[0] * i3];
    }
  }
  ...
  P = indices->size[0];
  i3 = c_indices->size[0] * c_indices->size[1];
  c_indices->size[0] = aux->size[1];
  c_indices->size[1] = P;
  emxEnsureCapacity((emxArray__common *)c_indices, i3, (int)sizeof(int));
  for (i3 = 0; i3 < P; i3++) {
    loop_ub = aux->size[1];
    for (i4 = 0; i4 < loop_ub; i4++) {
      c_indices->data[i4 + c_indices->size[0] * i3] = indices->data[i3 +
        indices->size[0] * (aux->data[aux->size[0] * i4] - 1)];
    }
  }
  ...
  emxEnsureCapacity((emxArray__common *)indices, i3, (int)sizeof(int));
  loop_ub = c_indices->size[1];
  for (i3 = 0; i3 < loop_ub; i3++) {
    P = c_indices->size[0];
    for (i4 = 0; i4 < P; i4++) {
      indices->data[i4 + indices->size[0] * i3] = c_indices->data[i4 +
        c_indices->size[0] * i3];
    }
  }
