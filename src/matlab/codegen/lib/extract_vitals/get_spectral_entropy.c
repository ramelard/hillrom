...
#include "relop.h"
#include "log21.h"
#include "extract_vitals_rtwutil.h"
...
void get_spectral_entropy(emxArray_creal_T *power_range, emxArray_creal_T *entr)
{
  ...
  if (power_range->size[0] == 1) {
    ...
    i9 = power_range->size[0] * power_range->size[1];
    power_range->size[0] = vlen;
    power_range->size[1] = 1;
    emxEnsureCapacity((emxArray__common *)power_range, i9, (int)sizeof(creal_T));
  }
  ...
  if ((power_range->size[0] == 0) || (power_range->size[1] == 0)) {
    ...
    vlen = y->size[1];
    for (i9 = 0; i9 < vlen; i9++) {
      y->data[y->size[0] * i9].re = 0.0;
      y->data[y->size[0] * i9].im = 0.0;
    }
  } else {
  ...
  emxEnsureCapacity((emxArray__common *)a, i9, (int)sizeof(creal_T));
  vlen = power_range->size[0] * power_range->size[1];
  for (i9 = 0; i9 < vlen; i9++) {
    a->data[i9] = power_range->data[i9];
  }
  ...
  if (!((power_range->size[0] == 0) || (power_range->size[1] == 0))) {
    ...
    i9 = av->size[0];
    av->size[0] = na1;
    emxEnsureCapacity((emxArray__common *)av, i9, (int)sizeof(creal_T));
    asub = 1;
    bsub = 1;
    ak = -1;
    bk = 0;
    nc1 = power_range->size[0];
    i9 = power_range->size[0] * power_range->size[1] - power_range->size[0];
    ck = 0;
  ...
  emxEnsureCapacity((emxArray__common *)r1, i9, (int)sizeof(boolean_T));
  vlen = power_range->size[0] * power_range->size[1];
  for (i9 = 0; i9 < vlen; i9++) {
    r1->data[i9] = rtIsNaN(power_range->data[i9].re);
  }
  ...
  emxEnsureCapacity((emxArray__common *)r2, i9, (int)sizeof(boolean_T));
  vlen = power_range->size[0] * power_range->size[1];
  for (i9 = 0; i9 < vlen; i9++) {
    r2->data[i9] = rtIsNaN(power_range->data[i9].im);
  }
  ...
  vlen = i9 * xoffset;
  for (i9 = 0; i9 < vlen; i9++) {
    r1->data[i9] = (r1->data[i9] || r2->data[i9]);
  }
  ...
  vlen = power_range->size[0] * power_range->size[1];
  for (i9 = 0; i9 < vlen; i9++) {
    r2->data[i9] = ((power_range->data[i9].re == 0.0) && (power_range->data[i9].
      im == 0.0));
  }
  ...
  vlen = r3->size[0];
  for (i9 = 0; i9 < vlen; i9++) {
    power_range->data[r3->data[i9] - 1].re = 1.0E-100;
    power_range->data[r3->data[i9] - 1].im = 0.0;
  }
  ...
  for (i9 = 0; i9 < 2; i9++) {
    uv0[i9] = (unsigned int)power_range->size[i9];
  }
  ...
  vlen = power_range->size[0] * power_range->size[1];
  for (i9 = 0; i9 < vlen; i9++) {
    s_re = power_range->data[i9].re;
    s_im = power_range->data[i9].im;
    brm = a->data[i9].re;
    y_re = a->data[i9].im;
    a->data[i9].re = s_re * brm - s_im * y_re;
    a->data[i9].im = s_re * y_re + s_im * brm;
  }

  i9 = entr->size[0] * entr->size[1];
  entr->size[0] = 1;
  entr->size[1] = a->size[1];
  emxEnsureCapacity((emxArray__common *)entr, i9, (int)sizeof(creal_T));
  if ((a->size[0] == 0) || (a->size[1] == 0)) {
    ...
    vlen = entr->size[1];
    for (i9 = 0; i9 < vlen; i9++) {
      entr->data[entr->size[0] * i9].re = 0.0;
      entr->data[entr->size[0] * i9].im = 0.0;
    }
  } else {
    vlen = a->size[0];
  ...
  vlen *= xoffset;
  for (i9 = 0; i9 < vlen; i9++) {
    s_im = -entr->data[i9].re;
    brm = -entr->data[i9].im;
    if (brm == 0.0) {
      entr->data[i9].re = s_im / s_re;
      entr->data[i9].im = 0.0;
    } else if (s_im == 0.0) {
      entr->data[i9].re = 0.0;
      entr->data[i9].im = brm / s_re;
    } else {
      entr->data[i9].re = s_im / s_re;
      entr->data[i9].im = brm / s_re;
    }
